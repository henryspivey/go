...
func VoteSurvey(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	voteSurvey(params["id"], params["voteIdx"])
...
func voteSurvey(survey string, voteIndex string) {
	id, _ := primitive.ObjectIDFromHex(survey)
	filter := bson.M{"_id": id}	
	update := bson.M{"$inc": bson.M{"votes."+voteIndex : 1}}
...
// Insert one survey in the DB
func insertOneVote(vote models.Vote) {
	insertResult, err := collection.InsertOne(context.Background(), vote)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("Inserted a Single Record ", insertResult)
